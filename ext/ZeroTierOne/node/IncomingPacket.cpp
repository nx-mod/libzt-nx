...
#ifdef __SWITCH__
					static unsigned int switch_auth_bad2 = 0;
					const unsigned int bad2_n = __sync_fetch_and_add(&switch_auth_bad2, 1);
					if (bad2_n < 100) {
						fprintf(stderr, "[SWITCH-AUTH] REJECT invalid-MAC(2) src=%llx verb=%u packet=%llx\n",
